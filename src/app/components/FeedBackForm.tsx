...
export default function FeedBackForm() {
    ...
    <div className="px-2">
        ...
        <div className="max-w-5xl m-auto flex flex-col space-y-4 border-2 border-indigo-500 rounded-md p-4 bg-gradient-to-r from-indigo-500 to-[#BA68C8]">
            ...
            <button onClick={async()=>{await sendFeedback(session?.user?.email, feedback, rating);setFeedback("");setRating("2.5");setPopUp(true)}} className="bg-gradient-to-tr border-2 border-white from-indigo-500 to-[#BA68C8] p-2 rounded-md w-fit mt-2">Submit</button>
        </div>
        :        
        <div className="max-w-5xl m-auto flex flex-col space-y-4 border-2 border-indigo-500 rounded-md p-4 bg-green-400">
            <button onClick={()=>setPopUp(false)} className="text-xl hover:text-gray-400 text-right w-fit self-end">X</button>
            <h1 className="text-center pb-10">Submitted!</h1>
        </div>
        }
    </div>
    )
